...
For additional stability, try `--label-smoothing`, `--focal-gamma`, and `--grad-clip`, and enable node type embeddings with `--type-embed-dim`. Training now also saves `cpg_best_metrics.json` and `cpg_test_metrics.json` in the output directory, and supports `--threshold`/`--threshold-grid` to tune the decision boundary.
